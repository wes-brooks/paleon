...
sink(paste("output/", sp, "-fulllocal.txt", sep=""))
print("running for: ")
print(sp)
...
neighbors = list()
for (i in 1:n) {
    neighbors[[i]] = which(D[i,]<=min(D[i,-i]))
}
...
bm.opt = function(theta, data, k1=25, k2=150) {
	...
	model = gam(biomass~s(composition, k=k1) + s(x,y,k=k2), data=data, gamma=1.4, family=Tweedie(p=theta, link='log'))
