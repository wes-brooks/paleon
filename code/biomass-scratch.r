...
for (sp in c(taxa[1])) {
    ...
    mi2 = inla(biomass.nz ~ f(loc, model='besag', graph=graphfile) + f(count, model='rw2'), data=modeldata, verbose=TRUE, family='Gamma')
    mi3 = inla(biomass.nz ~ f(loc, model='besag', graph=graphfile) + f(neighborhood.composition, model='rw2'), data=modeldata, verbose=TRUE, family='Gamma')
